...
DISK="$(df -m | grep '^/dev/' | awk '{use += $3; total += $2} END {printf("%.1f/%.1fGb (%d%%)", use/1024, total/1024, use/total*100)}')"
...
IP_MAC="$(hostname -I | awk '{print $1}') ($(ip link | awk '/link\/ether/ {print $2}' | head -n 1))"
